...
{"troubleshooting": "오늘 겪은 문제와 해결 과정을 한국어로 정리 (특별한 이슈가 없으면 빈 문자열)", "tomorrow": "기록을 바탕으로 다음에 이어서 하면 좋을 작업을 한 문장으로 제안 (근거가 부족하면 빈 문자열)"}`;
...
export class WorklogGeneratorService {
  ...
  async generate(projectId: string, date: string): Promise<GenerateWorklogResult> {
    ...
    if (context.eventCount === 0) {
      narrative = { troubleshooting: '', tomorrow: '' };
    } else {
      ...
      narrative = this.tryParseJson(raw) ?? { troubleshooting: raw, tomorrow: '' };
